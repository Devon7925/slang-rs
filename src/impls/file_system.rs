...
use crate::{Error, FileSystem, sys, utils};
...
unsafe extern "C" fn cast_as(this: *mut sys::ISlangCastable, guid: &sys::SlangUUID) -> *mut c_void {
	...
	if unsafe { query_interface(this.cast(), guid as *const _, &mut object) } == utils::S_OK {
		object
	} else {
		ptr::null_mut()
	}
...
unsafe extern "C" fn load_file(
	...
	let wrapper = unsafe { &mut (*this.cast::<FileSystemImpl>()).wrapper };
